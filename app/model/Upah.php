<?php
	...
	class Upah extends Database{

		private $table = "upah";
		private $primary = "upah_id";
		private $field = array(
									'upah_id',
									'jbtn_id',
									'upah_satuan',
	    ...
	    public function getDataUpah(){
	    	$result = $this->raw("SELECT * FROM `Upah` a JOIN `jabatan` b on a.`jbtn_id` = b.`jbtn_id`");
	     	return $result;
	    }

	    public function getDataUpahByID($id){
	    	$result = $this->raw("SELECT * FROM `Upah` a JOIN `jabatan` b on a.`jbtn_id` = b.`jbtn_id` where a.`upah_id` = '$id' ");
	     	return $result;
	    }
